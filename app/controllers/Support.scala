...
class Support(val controllerComponents: ControllerComponents, val wsClient: WSClient, config: Config) extends BaseController with Logging {
//    import pandaAuthActions.APIAuthAction
    ...
    def capiProxy(path: String) = Action.async { request =>
        query(s"${config.capiLiveUrl}/atom/guide/$path?api-key=${config.capiApiKey}&${request.rawQueryString}", Seq.empty)
    }

    def previewCapiProxy(path: String) = Action.async { request =>
